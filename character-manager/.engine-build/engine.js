...
exports.indexRuleset = indexRuleset;
exports.evaluate = evaluate;
exports.describeCondition = describeCondition;
exports.grantedTraits = grantedTraits;
exports.activeModifiers = activeModifiers;
exports.applyModifiers = applyModifiers;
exports.traitTierCost = traitTierCost;
exports.trackStepCost = trackStepCost;
exports.packageCost = packageCost;
exports.totalSpent = totalSpent;
exports.balances = balances;
exports.availableTraits = availableTraits;
exports.validate = validate;
function indexRuleset(ruleset) {
    return {
        ruleset,
 ...
 * ------------------------------------------------------------------ */
function evaluate(condition, character, idx) {
    switch (condition.kind) {
        ...
        case 'packageTier':
            return character.packageIds.some((id) => idx.packages.get(id)?.tier === condition.tier);
        case 'anyPackage':
 ...
 */
function describeCondition(condition, idx) {
    switch (condition.kind) {
        case 'always':
 ...
 */
function grantedTraits(character, idx) {
    const out = new Map();
    const walk = (grants) => {
...
function activeModifiers(character, idx) {
    const found = [];
    const collect = (grants) => {
        for (const g of grants) {
            if (g.kind === 'modifier')
                found.push(g.modifier);
            else if (g.kind === 'choice')
                collect(g.from);
        }
    };
    ...
    for (const [trackId, position] of Object.entries(character.trackPositions)) {
        ...
        for (const step of track.steps) {
            if (step.index <= position)
                collect(step.unlocks);
        }
    }
    return found.filter((m) => !m.activeWhen || evaluate(m.activeWhen, character, idx));
}
...
function matchesTarget(modifier, target, idx) {
    ...
    if (target.kind !== 'trait')
        return false;
    if (t.traitId !== undefined)
        return t.traitId === target.traitId;
    if (t.tag !== undefined) {
 ...
 */
function applyModifiers(base, target, character, idx) {
    const applicable = activeModifiers(character, idx).filter((m) => matchesTarget(m, target, idx));
    ...
    if (stacking === 'additive') {
        const pct = applicable
            ...
            .reduce((sum, m) => sum + (m.operation === 'percentReduction' ? m.value : -m.value), 0);
    ...
    for (const m of applicable)
        running = applyOne(running, m);
    return round(Math.max(0, running), rounding);
}
function applyOne(value, m) {
    switch (m.operation) {
 ...
 * Costs
 * ------------------------------------------------------------------ */
function traitTierCost(traitId, level, character, idx) {
...
function trackStepCost(trackId, stepIndex, character, idx) {
...
function packageCost(packageId, character, idx) {
    const pkg = idx.packages.get(packageId);
    if (!pkg)
        return null;
    return {
        currencyId: pkg.cost.currencyId,
        amount: applyModifiers(pkg.cost.amount, { kind: 'package', packageId }, character, idx),
    };
}
/** Total spent per currency, derived from what the character holds. */
function totalSpent(character, idx) {
    const spend = {};
    const add = (cost) => {
        if (!cost)
            return;
    ...
    for (const id of character.packageIds)
        add(packageCost(id, character, idx));
    const granted = grantedTraits(character, idx);
    for (const [traitId, level] of Object.entries(character.traitLevels)) {
        const free = granted.get(traitId) ?? 0;
        for (let l = free + 1; l <= level; l++)
            add(traitTierCost(traitId, l, character, idx));
    }
    for (const [trackId, position] of Object.entries(character.trackPositions)) {
        for (let s = 0; s <= position; s++)
            add(trackStepCost(trackId, s, character, idx));
    }
    return spend;
}
/** Awarded minus spent, per currency. */
function balances(character, idx) {
...
function groupChainAllows(groupId, character, idx) {
...
function capFor(trait, character, idx, phase) {
    const granted = grantedTraits(character, idx);
    let tightest = null;
    for (const rule of idx.ruleset.purchaseRules) {
        if (rule.phase !== 'both' && rule.phase !== phase)
            continue;
        if (rule.appliesTo.tag && !trait.tags.includes(rule.appliesTo.tag))
            continue;
        if (rule.appliesTo.groupId && trait.groupId !== rule.appliesTo.groupId)
            continue;
        if (rule.onlyIfNotGranted && granted.has(trait.id))
            continue;
...
function availableTraits(character, idx, phase = 'advancement') {
    const bal = balances(character, idx);
    return idx.ruleset.traits.map((trait) => {
        ...
        if (!nextTier)
            return { ...base, status: 'maxed', reason: 'At maximum level' };
        const groupCheck = groupChainAllows(trait.groupId, character, idx);
        if (!groupCheck.ok)
            return { ...base, status: 'locked', reason: groupCheck.reason };
 ...
 */
function validate(character, idx, phase = 'advancement') {
    const violations = [];
    // Package tier limits.
    for (const tier of idx.ruleset.packageTiers) {
        const held = character.packageIds.filter((id) => idx.packages.get(id)?.tier === tier.id);
